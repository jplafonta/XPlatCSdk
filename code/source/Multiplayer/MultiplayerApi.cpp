...
AsyncOp<MultiplayerModels::GetAssetDownloadUrlResponse> MultiplayerAPI::GetAssetDownloadUrl(
    const PlayFabMultiplayerGetAssetDownloadUrlRequest& request,
    const TaskQueue& queue
) const
{
    UnorderedMap<String, String> headers;
    ...
    return m_httpClient->MakePostRequest(
        ...
        JsonUtils::ToJson(request),
        queue
    ).Then([](Result<ServiceResponse> result) -> Result<GetAssetDownloadUrlResponse>
    {
        RETURN_IF_FAILED(result.hr);
