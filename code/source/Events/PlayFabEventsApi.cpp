...
) noexcept
{
    ...
    auto provider = MakeEntityProvider(async, __FUNCTION__, contextHandle->entity, std::bind(&EventsAPI::WriteEvents, &contextHandle->entity->eventsAPI, WriteEventsRequest{ *request }, std::placeholders::_1));
...
) noexcept
{
    ...
    *result = (PlayFabEventsWriteEventsResponse*)(std::dynamic_pointer_cast<WriteEventsResponse>((*resultHandle)->result).get());
...
) noexcept
{
    ...
    auto provider = MakeEntityProvider(async, __FUNCTION__, contextHandle->entity, std::bind(&EventsAPI::WriteTelemetryEvents, &contextHandle->entity->eventsAPI, WriteEventsRequest{ *request }, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
