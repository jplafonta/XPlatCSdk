...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&MatchmakerAPI::AuthUser, &contextHandle->state->matchmakerAPI, *request, std::placeholders::_1));
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&MatchmakerAPI::PlayerJoined, &contextHandle->state->matchmakerAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&MatchmakerAPI::PlayerLeft, &contextHandle->state->matchmakerAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&MatchmakerAPI::StartGame, &contextHandle->state->matchmakerAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&MatchmakerAPI::UserInfo, &contextHandle->state->matchmakerAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    *result = (PlayFabMatchmakerUserInfoResponse*)(std::dynamic_pointer_cast<UserInfoResponse>((*resultHandle)->result).get());
