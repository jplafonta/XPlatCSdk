...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AuthenticationAPI::ValidateEntityToken, &contextHandle->entity->authenticationAPI, *request, std::placeholders::_1));
...
) noexcept
{
    ...
    *result = (PlayFabAuthenticationValidateEntityTokenResponse*)(std::dynamic_pointer_cast<ValidateEntityTokenResponse>((*resultHandle)->model).get());
