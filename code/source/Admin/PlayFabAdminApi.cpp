...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AbortTaskInstance, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AddLocalizedNews, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AddNews, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AddPlayerTag, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AddServerBuild, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    *result = (PlayFabAdminAddServerBuildResult*)(std::dynamic_pointer_cast<AddServerBuildResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AddUserVirtualCurrency, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::AddVirtualCurrencyTypes, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::BanUsers, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminBanUsersResult*)(std::dynamic_pointer_cast<BanUsersResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CheckLimitedEditionItemAvailability, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreateActionsOnPlayersInSegmentTask, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreateCloudScriptTask, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreateInsightsScheduledScalingTask, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreateOpenIdConnection, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreatePlayerSharedSecret, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreatePlayerStatisticDefinition, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminCreatePlayerStatisticDefinitionResult*)(std::dynamic_pointer_cast<CreatePlayerStatisticDefinitionResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::CreateSegment, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteContent, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteMasterPlayerAccount, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminDeleteMasterPlayerAccountResult*)(std::dynamic_pointer_cast<DeleteMasterPlayerAccountResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteOpenIdConnection, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeletePlayer, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeletePlayerSharedSecret, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteSegment, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteStore, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteTask, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteTitle, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::DeleteTitleDataOverride, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ExportMasterPlayerData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetActionsOnPlayersInSegmentTaskInstance, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetActionsOnPlayersInSegmentTaskInstanceResult*)(std::dynamic_pointer_cast<GetActionsOnPlayersInSegmentTaskInstanceResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetAllSegments, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetAllSegmentsResult*)(std::dynamic_pointer_cast<GetAllSegmentsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetCatalogItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetCatalogItemsResult*)(std::dynamic_pointer_cast<GetCatalogItemsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetCloudScriptRevision, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetCloudScriptRevisionResult*)(std::dynamic_pointer_cast<GetCloudScriptRevisionResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetCloudScriptTaskInstance, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetCloudScriptTaskInstanceResult*)(std::dynamic_pointer_cast<GetCloudScriptTaskInstanceResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetCloudScriptVersions, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetCloudScriptVersionsResult*)(std::dynamic_pointer_cast<GetCloudScriptVersionsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetContentList, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetContentListResult*)(std::dynamic_pointer_cast<GetContentListResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetContentUploadUrl, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetDataReport, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetMatchmakerGameInfo, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetMatchmakerGameInfoResult*)(std::dynamic_pointer_cast<GetMatchmakerGameInfoResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetMatchmakerGameModes, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetMatchmakerGameModesResult*)(std::dynamic_pointer_cast<GetMatchmakerGameModesResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayedTitleList, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayedTitleListResult*)(std::dynamic_pointer_cast<GetPlayedTitleListResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerIdFromAuthToken, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerProfile, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayerProfileResult*)(std::dynamic_pointer_cast<GetPlayerProfileResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerSegments, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayerSegmentsResult*)(std::dynamic_pointer_cast<GetPlayerSegmentsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerSharedSecrets, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayerSharedSecretsResult*)(std::dynamic_pointer_cast<GetPlayerSharedSecretsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayersInSegment, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayersInSegmentResult*)(std::dynamic_pointer_cast<GetPlayersInSegmentResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerStatisticDefinitions, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayerStatisticDefinitionsResult*)(std::dynamic_pointer_cast<GetPlayerStatisticDefinitionsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerStatisticVersions, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayerStatisticVersionsResult*)(std::dynamic_pointer_cast<GetPlayerStatisticVersionsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPlayerTags, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPlayerTagsResult*)(std::dynamic_pointer_cast<GetPlayerTagsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPolicy, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPolicyResponse*)(std::dynamic_pointer_cast<GetPolicyResponse>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetPublisherData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetPublisherDataResult*)(std::dynamic_pointer_cast<GetPublisherDataResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetRandomResultTables, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetRandomResultTablesResult*)(std::dynamic_pointer_cast<GetRandomResultTablesResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetSegments, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetSegmentsResponse*)(std::dynamic_pointer_cast<GetSegmentsResponse>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetServerBuildInfo, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetServerBuildInfoResult*)(std::dynamic_pointer_cast<GetServerBuildInfoResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetServerBuildUploadUrl, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetStoreItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetStoreItemsResult*)(std::dynamic_pointer_cast<GetStoreItemsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetTaskInstances, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetTaskInstancesResult*)(std::dynamic_pointer_cast<GetTaskInstancesResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetTasks, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetTasksResult*)(std::dynamic_pointer_cast<GetTasksResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetTitleData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetTitleDataResult*)(std::dynamic_pointer_cast<GetTitleDataResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetTitleInternalData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserAccountInfo, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminLookupUserAccountInfoResult*)(std::dynamic_pointer_cast<LookupUserAccountInfoResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserBans, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetUserBansResult*)(std::dynamic_pointer_cast<GetUserBansResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetUserDataResult*)(std::dynamic_pointer_cast<GetUserDataResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserInternalData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserInventory, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGetUserInventoryResult*)(std::dynamic_pointer_cast<GetUserInventoryResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserPublisherData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserPublisherInternalData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserPublisherReadOnlyData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GetUserReadOnlyData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::GrantItemsToUsers, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminGrantItemsToUsersResult*)(std::dynamic_pointer_cast<GrantItemsToUsersResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::IncrementLimitedEditionItemAvailability, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::IncrementPlayerStatisticVersion, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminIncrementPlayerStatisticVersionResult*)(std::dynamic_pointer_cast<IncrementPlayerStatisticVersionResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ListOpenIdConnection, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminListOpenIdConnectionResponse*)(std::dynamic_pointer_cast<ListOpenIdConnectionResponse>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ListServerBuilds, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminListBuildsResult*)(std::dynamic_pointer_cast<ListBuildsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ListVirtualCurrencyTypes, &contextHandle->state->adminAPI, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminListVirtualCurrencyTypesResult*)(std::dynamic_pointer_cast<ListVirtualCurrencyTypesResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ModifyMatchmakerGameModes, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ModifyServerBuild, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminModifyServerBuildResult*)(std::dynamic_pointer_cast<ModifyServerBuildResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RefundPurchase, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RemovePlayerTag, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RemoveServerBuild, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RemoveVirtualCurrencyTypes, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ResetCharacterStatistics, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ResetPassword, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ResetUserStatistics, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::ResolvePurchaseDispute, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RevokeAllBansForUser, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminRevokeAllBansForUserResult*)(std::dynamic_pointer_cast<RevokeAllBansForUserResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RevokeBans, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminRevokeBansResult*)(std::dynamic_pointer_cast<RevokeBansResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RevokeInventoryItem, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RevokeInventoryItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminRevokeInventoryItemsResult*)(std::dynamic_pointer_cast<RevokeInventoryItemsResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::RunTask, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SendAccountRecoveryEmail, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetCatalogItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetPlayerSecret, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetPublishedRevision, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetPublisherData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetStoreItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetTitleData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetTitleDataAndOverrides, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetTitleInternalData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SetupPushNotification, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::SubtractUserVirtualCurrency, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateBans, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminUpdateBansResult*)(std::dynamic_pointer_cast<UpdateBansResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateCatalogItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateCloudScript, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateOpenIdConnection, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdatePlayerSharedSecret, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdatePlayerStatisticDefinition, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminUpdatePlayerStatisticDefinitionResult*)(std::dynamic_pointer_cast<UpdatePlayerStatisticDefinitionResult>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdatePolicy, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    RETURN_IF_FAILED(XAsyncGetResult(async, nullptr, sizeof(PlayFabResultHandle), resultHandle, nullptr));
    *result = (PlayFabAdminUpdatePolicyResponse*)(std::dynamic_pointer_cast<UpdatePolicyResponse>((*resultHandle)->model).get());
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateRandomResultTables, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateSegment, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateStoreItems, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateTask, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserInternalData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserPublisherData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserPublisherInternalData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserPublisherReadOnlyData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserReadOnlyData, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, std::bind(&AdminAPI::UpdateUserTitleDisplayName, &contextHandle->state->adminAPI, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
