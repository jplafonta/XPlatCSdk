...
) const
{
    ...
    headers.emplace("X-Authorization", m_tokens->SessionTicket);
...
AsyncOp<ClientModels::GetWindowsHelloChallengeResponse> ClientAPI::GetWindowsHelloChallenge(
...
)
{
    ...
    return httpClient->MakePostRequest(
        ...
        JsonUtils::ToJson(request),
        queue
    ).Then([](Result<ServiceResponse> result) -> Result<GetWindowsHelloChallengeResponse>
    {
        RETURN_IF_FAILED(result.hr);
        ...
        {
            return Result<GetWindowsHelloChallengeResponse>{ ServiceErrorToHR(serviceResponse.ErrorCode), std::move(serviceResponse.ErrorMessage) };
        }
    });
}
...
AsyncOp<void> ClientAPI::LinkWindowsHello(
...
AsyncOp<void> ClientAPI::UnlinkWindowsHello(
